...
public class HailParticle extends WeatherParticle {
    ...
    public HailParticle(ClientLevel level, double x, double y, double z) {
    ...
    public void doCollisionEffects(BlockHitResult hitResult) {
        if (!hitResult.getLocation().closerThan(Minecraft.getInstance().cameraEntity.position(), CONFIG.perf.particleDistance * CONFIG.perf.impactParticleDistance)) {
            return;
        }
        ...
        if (hitResult.getDirection().getAxis().isVertical()) {
            ...
            if (hit.getType().equals(HitResult.Type.MISS) && collisionBelowExists) {
                ...
                if (state.is(BlockTags.INFINIBURN_OVERWORLD) || state.is(BlockTags.STRIDER_WARM_BLOCKS)) {
                    ...
                    spawnPos = new Vec3(spawnPos.x, Mth.floor(spawnPos.y) + state.getFluidState().getOwnHeight(), spawnPos.z);
                }
            ...
            collisionPos = collisionPos.relative(hitResult.getDirection(), 0.02);
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float tickPercentage) {
        ...
        // angle particle along axis of relative velocity
        final Vector3f camD = Minecraft.getInstance().cameraEntity.getDeltaMovement().toVector3f();
        Vector3f delta = new Vector3f((float) this.xd - (camD.x), (float) this.yd - (camD.y), (float) this.zd - (camD.z));
        ...
        quaternion = this.turnBackfaceFlipways(quaternion, new Vector3f(x, y, z));
